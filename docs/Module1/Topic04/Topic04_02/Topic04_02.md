...
取索引只能获取字符串中的单个字符，如果想获得多个字符，或者说获得字符串的一个**字串**，就要用到**切片（slice）**了。
...
- (1) 如果不规定步长，步长就为默认1，这时的切片写法就是 `字符串[开始索引:结束索引]`，第二个冒号可以省略，例如：
...
- (2) 如果规定了步长，切片写法就必须完整 `字符串[开始索引:结束索引:步长]`，例如
