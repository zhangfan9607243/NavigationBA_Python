...
我们本节使用一个名为 `data1.json` 的 JSON 文件作为例子，文件内容如下：
...
之后我们可以打开 `data2.json` 文件查看写入的内容，内容如下：
...
- 我们这节使用一个 `student1.csv` 的文件，打开我们就可以看到 CSV 文件本来的形式：
...
之后我们可以打开 `student2.csv` 文件查看写入的内容，内容如下：
