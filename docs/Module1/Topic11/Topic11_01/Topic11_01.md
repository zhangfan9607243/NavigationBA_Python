...
在本节，我们假设我们有一个文本文件，文件名为`咏鹅.txt`，内容如下：
...
我们来尝试自己在 Python 中写一遍《咏鹅》，然后把它写入一个文本文件中：
...
运行这段代码后，会在当前目录下生成一个名为 `咏鹅1.txt` 的文本文件，我们点开可以看到内容如下：
...
如果我们再以**覆盖式写入**的方式打开 `咏鹅1.txt` 文件，并写入新的内容：
...
运行这段代码后，`咏鹅1.txt` 文件的内容会被新的内容覆盖，变成：
...
我们自己在 Python 中写一遍《咏鹅》，来尝试使用追加式写入的方式，先写入到 `咏鹅2.txt` 文件中：
...
如果我们再以**追加式写入**的方式打开 `咏鹅2.txt` 文件，并写入新的内容：
...
我们来尝试使用 `writelines()` 方法，来将《咏鹅》逐行写入到 `咏鹅3.txt` 文件中：
...
之后我们再尝试用追加式写入的方式，来将新的内容逐行写入到 `咏鹅3.txt` 文件中：
